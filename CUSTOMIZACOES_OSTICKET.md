...
| 2026-08-25 | **NOVO** `include/class.turnstile.php` + `bootstrap.php` (core) + `include/ost-config.php` + `account.php`/`include/client/register.inc.php` + `open.php`/`include/client/open.inc.php` + `docker/Dockerfile` (extensão `curl`) + `docker/apache-osticket.conf` + `docker-compose.yml`/`.env*` | **Endurecimento de segurança para o piloto público**: CAPTCHA **Cloudflare Turnstile** (chaves via `.env`, desligado se vazias) na criação de conta (porta pública real hoje, já que `clients_only=1` exige login antes de abrir chamado) e na abertura de chamado (substitui o CAPTCHA de imagem legado do core, dormente hoje); `session.cookie_secure` condicional a `is_https()`; `TRUSTED_PROXIES` parametrizável via `OST_TRUSTED_PROXIES` (ranges do Cloudflare); cabeçalhos `X-Content-Type-Options`/`X-Frame-Options`/`Referrer-Policy`/HSTS condicional no Apache. Auditoria de SQL Injection no código customizado (`zk_equipment.php`, `zk-equip-edit.php`, `scp/zk-equip.php`): 100% dos `db_query()` já usam `db_input()` — nenhuma correção necessária. | [x] aplicado e testado (php -l) |
| 2026-08-25 | `db/migrations/004-garantia-equipamento.sql` (nova coluna `ost_zk_equipment.garantia`) + `include/zk_equipment.php` + `scp/zk-equip.php` + `include/client/open.inc.php` | **Campo "Garantia" por equipamento**: na abertura do chamado, o cliente escolhe "Solicitar garantia" ou "Não possuo garantia" por item (cada um com seu próprio Nº de Série); ao solicitar, nasce como **"Garantia em análise"** — só o agente (painel do agente, depois de receber o equipamento e confirmar o Nº de Série) pode aprovar/recusar. Allowlist no servidor impede o cliente de forjar `aprovada`/`recusada` direto no JSON de abertura. Badge "Garantia" visível nos painéis do cliente e do agente; mudança de decisão do agente vira resposta na thread + e-mail ao cliente (reaproveita `zk_equip_notify_changes()`, já existente). | [x] aplicado e testado (php -l + migração aplicada no banco vivo) |
| 2026-08-26 | **NOVO** `assets/documents/politica-manutencao-cliente-final.pdf` + `assets/documents/politica-manutencao-revendedor.pdf` (PDFs servidos estaticamente) + `account.php` (core) + `include/client/register.inc.php` + `assets/default/css/theme.css` + `db/migrations/005-aceite-politicas-manutencao.sql` (nova tabela `ost_zk_terms_acceptance`) | **Aceite obrigatório das Políticas de Manutenção no primeiro acesso (criação de conta)**: checkbox obrigatório na tela de registro ("Li e concordo...") com links para os 2 PDFs (cliente final e revendedor), validado no servidor (`account.php`, antes da criação da conta — mesmo padrão dos outros `elseif` de validação) e sem o qual a conta não é criada. Ao criar a conta com sucesso, grava prova auditável em `ost_zk_terms_acceptance` (user_id, e-mail, documentos, IP, user-agent, data/hora) — objetivo é ter registro formal do aceite para evitar contestação em caso de desistência do procedimento de manutenção. Não se aplica a clientes já com conta (edição de perfil) nem a login recorrente — é só na criação da conta (verdadeiro "primeiro acesso"). | [-] substituído pela unificação abaixo (ver evento 2026-08-26 seguinte) |
| 2026-08-26 | **Unificação dos 2 PDFs em 1 termo único** — `assets/documents/termo-manutencao-zkteco.html` (NOVO, substitui os 2 PDFs anteriores, removidos) + `include/client/register.inc.php` (link único) + `account.php` (valor gravado em `documents` da tabela de auditoria) | Usuário mesclou os dois documentos (cliente final + revendedor) em um só, fornecido como `Termo_Unico_Manutencao_ZKTeco.md` (raiz do projeto, fora do `app/`, no mesmo espírito deste changelog). Convertido manualmente para uma página HTML estática (mesma pasta `assets/documents/`, sem dependência de conversor — não há `pandoc`/`python`/similar no container) com estilo ZKTeco leve, aberta em nova aba pelo checkbox de aceite. O checkbox e a validação (client-side obrigatório + server-side em `account.php`) continuam iguais, só a referência ao documento mudou. Se o `.md` fonte for editado de novo, reaplicar manualmente o conteúdo no `.html` (não há build automatizado). | [-] substituído pelo modal de leitura obrigatória (evento seguinte) |
| 2026-08-26 | **Modal de leitura obrigatória + resumo de valores em destaque** — **NOVO** `include/client/zk-termo-manutencao-content.inc.php` (fonte única do conteúdo do termo) + `assets/documents/termo-manutencao-zkteco.php` (substitui o `.html` estático — agora inclui o parcial) + `include/client/register.inc.php` (caixa de destaques + modal + JS) + `assets/default/css/theme.css` + `account.php` (grava `scrolled_confirmed`) + `db/migrations/006-scroll-confirmado-termo.sql` (nova coluna `ost_zk_terms_acceptance.scrolled_confirmed`) | Pedido do cliente para reforçar a evidência e evitar "problema de comunicação": (1) caixa `.zk-terms-highlights` sempre visível na tela de registro, **antes** do aceite, resumindo garantia (12 meses hardware / 3 meses pós-reparo), valores de reparo fora da garantia (orçamento + taxa de R$ 90 se reprovado/sem defeito) e armazenagem (R$ 1,00/dia após 5 dias úteis) — confirmado com o cliente que "valores de embalagem" = armazenagem (não existe taxa de embalagem separada em nenhum documento); (2) o link do termo abre um **modal** (`#zkTermsOverlay`) com o texto integral rolável; o botão "Li e concordo com o termo" só habilita ao rolar até o fim, e só ele destrava o checkbox principal (que nasce `disabled` via JS). Progressive enhancement: sem JavaScript, o checkbox volta a ser um checkbox comum obrigatório e o link abre o termo em nova aba (fallback natural do `href`). `scrolled_confirmed` é só reforço de evidência (não é controle de segurança — vem de campo oculto client-side). | [x] aplicado e testado (php -l + HTTP 200 + fluxo completo validado via browser headless: checkbox trava até o aceite no modal, botão só habilita após rolar ao fim) |
| 2026-08-27 | Banco (`ost_form_field`, form_id=1 — Usuário; novos ids 43/44) + `include/client/register.inc.php` + `assets/default/css/theme.css` | **Campos "Data de Nascimento" e "CPF ou CNPJ" no cadastro** (obrigatórios, para identificação do cliente) + **destaque visual no termo de aceite**: caixa do termo pisca vermelho suave se o cliente tentar marcar "aceito" sem antes abrir o link do termo (checkbox nasce `disabled` até isso — ver evento 2026-08-26) | [x] aplicado e testado (php -l + HTTP 200 + validação regex CPF/CNPJ conferida via PHP) |
| 2026-08-27 | `include/client/register.inc.php`, `include/client/profile.inc.php`, `assets/default/css/theme.css` | **Calendário do campo "Data de Nascimento" no estilo Google (Material)**: o widget é o mesmo jQuery UI datepicker do core (`class.forms.php`, `DatetimePickerWidget`), só reconfigurado via JS por cima (sem editar o core) — painel verde no topo com a data por extenso ("Qui, 27 de ago"), 1 mês por vez (core pedia 2), dias em círculo (hoje com contorno verde, selecionado preenchido) e rodapé **Limpar / Cancelar / Definir** substituindo os botões nativos "Today/Done". Mira a classe fixa `dp` que o widget sempre emite (nome/id do campo são hash por sessão, não dá pra mirar por eles). Cancelar restaura o valor que o campo tinha ao abrir o calendário (útil se só navegou os meses sem escolher dia); como o clique num dia já preenche e fecha o calendário na hora (1 toque só, mais simples pro público leigo), Cancelar/Definir na prática só diferem se o valor foi digitado à mão enquanto o calendário estava aberto. | [x] aplicado e testado (php -l + browser headless via Puppeteer: hero/botões renderizam, Limpar/Cancelar validados com o valor do campo) |
