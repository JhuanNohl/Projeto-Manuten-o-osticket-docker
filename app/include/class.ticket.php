...
implements RestrictedAccess, Threadable, Searchable {
    ...
    function setStatus($status, $comments='', &$errors=array(), $set_closing_agent=true, $force_close=false) {
        ...
        /* ZK-EQUIP:BEGIN — notifica o cliente por e-mail a CADA troca de
           status (só chamados com equipamentos; $hadStatus pula o status
           inicial da criação — o autoresponder já cobre esse). A
           notificação é reflexo: nunca bloqueia a troca. */
        if ($hadStatus && function_exists('zk_notify_status_change')) {
            try { zk_notify_status_change($this, $status); }
