    ...
    function editField($tid, $fid) {
        ...
        if ($_POST && $form->isValid()) {
            ...
            if ($ticket->updateField($form, $errors)) {
                // ZK: ao editar a "Data de Vencimento", recalcula o atraso na
                // hora (a flag isoverdue só era atualizada pelo cron/monitor de
                // SLA — por isso o card "ATRASADO!" persistia/demorava). O card
                // é re-renderizado pelo reload disparado no client (ticket-view).
                if ($fid === 'duedate' && ($t2 = Ticket::lookup($tid))) {
                    $due = $t2->getDueDate() ?: $t2->getEstDueDate();
                    if ($t2->isOpen() && $due && Misc::db2gmtime($due) < Misc::gmtime())
                        $t2->markOverdue(false);   // false = não dispara alerta
                    else
                        $t2->clearOverdue(true);
                }
                $msg = sprintf(
                      __('%s successfully'),
                      sprintf(
                ...
                switch (true) {
                    ...
                    case $field instanceof FileUploadField:
                        ...
                        $clean = $answer->display() ?: '&mdash;' . str_replace('Esvaziar', 'Vazio', __('Empty')) . '&mdash;'; // ZK: "Esvaziar"->"Vazio"
                        break;
                    ...
                    case $field instanceof TextareaField:
                        ...
                        $clean = Format::striptags($clean) ?: '&mdash;' . str_replace('Esvaziar', 'Vazio', __('Empty')) . '&mdash;'; // ZK: "Esvaziar"->"Vazio"
                        if (strlen($clean) > 200)
                ...
                $response = array(
                    'value' => $clean ?: '&mdash;' . str_replace('Esvaziar', 'Vazio', __('Empty')) . '&mdash;', // ZK: "Esvaziar"->"Vazio"
                    'id' => $fid, 'msg' => $msg
                );
