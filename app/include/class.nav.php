    ...
    function getTabs(){
        ...
        if(!isset($this->tabs)) {
            ...
            $this->tabs['dashboard'] = array(
                'desc'=>__('Visão geral'),'href'=>'dashboard.php','title'=>__('Agent Dashboard'), "class"=>"no-pjax"
            );
            $this->tabs['tickets'] = array('desc'=>__('Manutenções'),'href'=>'tickets.php','title'=>__('Ticket Queue'));
            if ($thisstaff->hasPerm(User::PERM_DIRECTORY)) {
                $this->tabs['users'] = array(
                    'desc' => __('Clientes'), 'href' => 'users.php', 'title' => __('User Directory')
                );
            }
            // ZK: mantidas no array (páginas próprias dependem da chave existir
            // em setTabActive), mas fora do menu superior — só Visão geral,
            // Manutenções e Clientes devem aparecer.
            $this->tabs['tasks'] = array('desc'=>__('Tasks'), 'href'=>'tasks.php', 'title'=>__('Task Queue'), 'hidden'=>true);
            $this->tabs['kbase'] = array('desc'=>__('Knowledgebase'),'href'=>'kb.php','title'=>__('Knowledgebase'), 'hidden'=>true);
            if (!is_null($this->getRegisteredApps()))
                $this->tabs['apps']=array('desc'=>__('Applications'),'href'=>'apps.php','title'=>__('Applications'), 'hidden'=>true);
        }
        ...
        foreach($this->getTabs() as $k=>$tab){
            ...
            switch(strtolower($k)){
                case 'tasks':
                    ...
                    break;
                case 'dashboard':
                    $subnav[]=array('desc'=>__('Visão geral'),'href'=>'dashboard.php','iconclass'=>'logs');
                    ...
                    break;
                case 'users':
                    $subnav[] = array('desc' => __('Clientes'), 'href' => 'users.php', 'iconclass' => 'teams');
                    $subnav[] = array('desc' => __('Empresas'), 'href' => 'orgs.php', 'iconclass' => 'departments');
                    break;
                case 'kbase':
...
class UserNav {
    ...
    function getNavLinks(){
        ...
        if(!$this->navs){
            ...
            $navs = array();
            $user = $this->user;
            // Sem mais "Página Principal": o portal do cliente não tem uma
            // home separada — só Abrir Novo Ticket e Chamados (ou o login).
            if($cfg && $cfg->isKnowledgebaseEnabled())
                $navs['kb']=array('desc'=>__('Knowledgebase'),'href'=>'kb/index.php','title'=>'');

            // "Abrir Novo Ticket" só aparece pra quem já está logado de
            // verdade (conta registrada, não convidado) — sem login não há
            // nenhum item de menu: o cliente deve estar na tela de login.
            // "Verificar Status do Ticket" (busca manual por e-mail+número)
            // foi removida por completo: sem uso para esta solução, já que
            // agora é sempre exigido login para abrir/acompanhar chamado.
