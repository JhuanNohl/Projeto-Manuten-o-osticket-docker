...
abstract class AuthenticationBackend extends ServiceRegistry {
    ...
    static function process($username, $password=null, &$errors=array()) {
        ...
        // Negativas emitidas por um backend (bloqueio por excesso de
        // tentativas, política etc.) trazem um motivo específico que deve
        // ser exibido. Credencial simplesmente incorreta não preenche
        // $errors['err'] — cada tela de login define a mensagem adequada
        // ao seu contexto (usuário/senha ou e-mail/nº do chamado).
        if ($result && $result instanceof AccessDenied)
            $errors['err'] = $result->reason;
        elseif (!$result)
            // AccessDenied ainda é criado para a auditoria abaixo contar a
            // tentativa falha (AuthStrikeBackend).
