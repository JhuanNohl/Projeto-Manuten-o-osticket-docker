...
// each attempt.
if ($_POST) {
    // Token inválido/expirado (página aberta há muito tempo ou em outra
    // aba): descarta o POST e reexibe o login — já com token novo — com
    // uma orientação amigável em vez do erro técnico de CSRF.
    if (!$ost->checkCSRFToken()) {
        $errors['err'] = __('Sua sessão expirou. Por favor, tente novamente.');
        $_POST = array();
    }
    else
...
if ($_POST && isset($_POST['luser'])) {
    ...
    elseif (Validator::is_userid(trim($_POST['luser']), $errors['err'], false)
        ...
        // Exibida dentro do cartão de login (.zk-login-err), junto aos campos
        $errors['login'] = __('Usuário ou senha incorretos');
    }
...
// Tela de login é a porta de entrada única do portal: sem menu de
// navegação (nada de "Página Principal"/"Abrir Novo Ticket"/"Verificar
// Status" aqui). O cliente precisa entrar (ou criar conta) primeiro.
$nav = false;
