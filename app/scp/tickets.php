...
if (!$ticket) {
    ...
    // ZK: Filtro avançado INLINE (substitui o popup "Pesquisa Avançada").
    // Traduz os parâmetros zk_* da barra de filtro em critérios de busca
    // ad-hoc e reusa TODO o pipeline nativo (AdhocSearch -> mangleQuerySet).
    // Formatos: ChoiceField => [nome,'includes',[id=>id]];
    //           DatetimeField => [nome,'between'|'after'|'before', ...];
    //           palavra-chave => [':keywords', null, termo].
    elseif (isset($_GET['a']) && $_GET['a'] === 'zksearch') {
        $criteria = array();

        // palavra-chave (mesma regra da busca simples: até 3 termos)
        if (($q = trim($_GET['zk_q'] ?? '')) !== '' && mb_str_wc($q) < 4)
            $criteria[] = array(':keywords', null, $q);

        // campos de escolha (dropdowns) -> método 'includes'
        $zk_choice = array(
            'zk_status'   => 'status__id',
            'zk_state'    => 'status__state',
            'zk_dept'     => 'dept_id',
            'zk_assignee' => 'staff_id',
            'zk_topic'    => 'topic_id',
            'zk_sla'      => 'sla_id',
        );
        foreach ($zk_choice as $param => $field) {
            $raw = $_GET[$param] ?? '';
            $vals = is_array($raw) ? $raw : ($raw === '' ? array() : array($raw));
            $vals = array_filter($vals, function ($x) { return $x !== '' && $x !== null; });
            if ($vals) {
                $assoc = array();
                foreach ($vals as $x) $assoc[$x] = $x;
                $criteria[] = array($field, 'includes', $assoc);
            }
        }

        // intervalos de data -> between / after / before
        $zk_dates = array(
            'created' => array('zk_created_from', 'zk_created_to'),
            'duedate' => array('zk_due_from', 'zk_due_to'),
        );
        foreach ($zk_dates as $field => $pp) {
            $from = trim($_GET[$pp[0]] ?? '');
            $to   = trim($_GET[$pp[1]] ?? '');
            if ($from !== '' && $to !== '')
                $criteria[] = array($field, 'between', array('left' => $from, 'right' => $to));
            elseif ($from !== '')
                $criteria[] = array($field, 'after', $from);
            elseif ($to !== '')
                $criteria[] = array($field, 'before', $to);
        }

        if ($criteria) {
            $_SESSION['advsearch']['zkadv'] = $criteria;
            $queue_id = 'adhoc,zkadv';
        } else {
            // Filtro vazio (tudo "Qualquer"): não cria busca — limpa qualquer
            // busca ZK anterior e volta para a fila padrão (senão ficaria numa
            // adhoc vazia/obsoleta mostrando "nada").
            unset($_SESSION['advsearch']['zkadv']);
            $queue_id = $thisstaff->getDefaultTicketQueueId() ?: $cfg->getDefaultTicketQueueId();
        }
    }
    ...
    // ZK: buscas ad-hoc caem nas "colunas padrão de último recurso" que NÃO
    // incluem a coluna Status. Faz a fila da busca herdar as colunas da fila
    // padrão do agente (que têm Status) — precisa ser ANTES do getQuery() para
    // que a query anote/selecione os dados do Status (getQuery usa getColumns).
    if ($queue && strpos((string) $queue->getId(), 'adhoc') === 0) {
        $zk_col_qid = $thisstaff->getDefaultTicketQueueId() ?: $cfg->getDefaultTicketQueueId();
        if ($zk_col_qid)
            $queue->set('columns_id', $zk_col_qid);
    }
    ...
    if($ticket && $ticket->getId()) {
        ...
        case 'reply':
            if (!$role || !$role->hasPerm(Ticket::PERM_REPLY)) {
                ...
                // ZK: Cc como e-mails livres (substitui o antigo seletor de
                // colaboradores) — cada endereço vira (ou reaproveita) um
                // usuário e entra como colaborador ativo desta resposta.
                $vars['ccs'] = array();
                if ($vars['cc_emails'] && ($vars['cc_emails'] = trim($vars['cc_emails']))) {
                    foreach (preg_split('/[,;]+/', $vars['cc_emails']) as $ccEmail) {
                        if (!($ccEmail = trim($ccEmail)))
                            continue;
                        if (!Validator::is_email($ccEmail)) {
                            $errors['cc_emails'] = __('One or more Cc email addresses is invalid');
                            continue;
                        }
                        if ($ccUser = User::fromVars(array('email' => $ccEmail)))
                            $vars['ccs'][] = $ccUser->getId();
                    }
                    $vars['ccs'] = array_unique($vars['ccs']);
                }
            }
...
// ZK: chips "Etapas Operacionais"/"Finalizados" removidos do submenu
// (2026-08-21) — poluíam a barra pro colaborador e o filtro avançado
// (campo Estado/Status) já cobre a mesma necessidade. $queue continua
// resolvido normalmente (fila padrão do agente/sistema, ver acima),
// só a navegação por chips deixou de ser renderizada.

// ZK: item "Pesquisar" (buscas salvas + busca avançada) removido do
// submenu — a busca básica continua disponível pelo campo sempre visível
// no cabeçalho (include/staff/header.inc.php) e pelo campo já existente
// dentro da própria fila (queue-tickets.tmpl.php).

if ($thisstaff->hasPerm(Ticket::PERM_CREATE, false)) {
    $nav->addSubMenu(array('desc'=>__('Novo RMA'),
