...
// Mensagem de autenticação é one-shot: sem o unset, um aviso antigo (ex.:
// falha de CSRF) fica "grudado" e reaparece em todo acesso à tela,
// mascarando o erro real da tentativa atual.
unset($_SESSION['_staff']['auth']['msg']);
// ZK: mensagem padrão da tela de login trocada pelo contexto do painel
// (a original, "Authentication Required", é uma string core traduzida via
// pt_BR.phar — não dá pra editar o texto direto, então sobrescrevemos o
// fallback aqui). Mensagens de sessão (erro de login) e a página "banner-staff"
// continuam tendo prioridade — isso só entra quando nenhuma das duas existe.
$msg = $msg ?: ($content ? $content->getLocalName() : __('Painel da Equipe de Manutenção'));
...
if ($_POST) {
    ...
    if (!$ost->checkCSRFToken()) {
        // Token expirado/antigo (aba aberta há muito tempo): volta para a
        // tela — já com token novo — orientando a tentar de novo, em vez
        // do erro técnico de CSRF.
        $_SESSION['_staff']['auth']['msg'] = __('Sua sessão expirou. Por favor, tente novamente.');
...
if ($_POST && isset($_POST['userid'])) {
    ...
    $msg = $errors['err'] ?: __('Usuário ou senha incorretos');
    $show_reset = true;
...
elseif (!$thisstaff || !($thisstaff->getId() || $thisstaff->isValid())) {
    if (($user = StaffAuthenticationBackend::processSignOn($errors, false))
        ...
        // Backends externos (ex.: OAuth) podem gravar a mensagem durante o
        // processSignOn() acima — também é one-shot.
