...
(function(R$) {
  ...
  /* ZK: tamanho de fonte — mesmo padrão do plugin 'fontfamily' nativo
     (redactor-plugins.js), só troca a propriedade CSS aplicada. */
  R$.add('plugin', 'fontsize', {
    ...
    init: function (app) {
        ...
        this.sizes = this.opts.fontsizes ||
    ...
    start: function() {
        ...
        dropdown.remove = {
            title: this.lang.get('remove-font-size'),
            api: 'plugin.fontsize.remove'
        };
        var $button = this.toolbar.addButtonAuto('fontsize', {
            title: this.lang.get('fontsize')
        });
        $button.setIcon('<i class="icon-text-height"></i>');
  ...
  /* ZK: alinhamento de parágrafo — mesma técnica do plugin nativo
     'textdirection' (bloco atual via selection.getBlock()), aplicando
     text-align em vez do atributo dir. */
  R$.add('plugin', 'textalign', {
    translations: {
      en: {
        "textalign": "Alignment",
        "align-left": "Left",
    ...
    start: function() {
        var dropdown = {
            left: {
                title: '<i class="icon-align-left"></i> ' + this.lang.get('align-left'),
                api: 'plugin.textalign.set', args: 'left'
            },
            center: {
                title: '<i class="icon-align-center"></i> ' + this.lang.get('align-center'),
                api: 'plugin.textalign.set', args: 'center'
            },
            right: {
                title: '<i class="icon-align-right"></i> ' + this.lang.get('align-right'),
                api: 'plugin.textalign.set', args: 'right'
            },
            justify: {
                title: '<i class="icon-align-justify"></i> ' + this.lang.get('align-justify'),
                api: 'plugin.textalign.set', args: 'justify'
            }
        };
        var $button = this.toolbar.addButtonAuto('textalign', {
            title: this.lang.get('textalign')
        });
        $button.setIcon('<i class="icon-align-left"></i>');
        $button.setDropdown(dropdown);
    },
    set: function(type) {
        var block = this.selection.getBlock();
        if (block)
            $R.dom(block).css('text-align', type);
    }
  });
})(Redactor);
    ...
    redact = $.fn.redact = function(el, options) {
        ...
        var options = $.extend({
                ...
                'buttons': el.hasClass('no-bar')
                  ...
                  // ZK: caixa de resposta do chamado/nota (cliente e agente) —
                  // toolbar com o que serve para uma correspondência de
                  // suporte (fonte/tamanho/cor, negrito/itálico/sublinhado/
                  // tachado, alinhamento, listas, link, imagem); tira recursos
                  // de autoria de documento (vídeo, arquivo, tabela, linha
                  // horizontal, ver código-fonte) irrelevantes aqui. Outros
                  // editores richtext do painel (respostas prontas, FAQ,
                  // e-mails) continuam com a toolbar completa.
                  : el.hasClass('zk-basic-editor')
                  ? ['format', 'fontfamily', 'fontsize', 'fontcolor', 'bold',
                    'italic', 'underline', 'deleted', 'textalign', 'lists',
                ...
                'plugins': el.hasClass('no-bar')
                  ...
                  : el.hasClass('zk-basic-editor')
                  ? ['imagemanager','definedlinks','autolock','fontcolor',
